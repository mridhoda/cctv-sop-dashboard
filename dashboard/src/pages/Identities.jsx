...
// TODO: GET /api/identities - POST /api/identities - DELETE /api/identities/{id}
// TODO: POST /api/identities/{id}/encode for face encoding
...
export default function Identities() {
  ...
  const tidakAktif = identities.filter(i => i.status === 'Inactive').length;

  const handleAddIdentity = () => {
    if (formData.namaLengkap && formData.jabatan && formData.idKaryawan) {
      const newIdentity = {
        id: `ID${String(identities.length + 1).padStart(3, '0')}`,
      ...
      setFormData({ namaLengkap: '', jabatan: '', idKaryawan: '', foto: null });
      ...
      <div className="sticky top-0 bg-white border-b border-slate-200 z-40">
        <div className="px-8 py-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-bold text-slate-900">Manajemen Identitas</h1>
              <p className="text-slate-600 text-sm mt-1">Kelola data identitas dan wajah staff terdaftar</p>
            </div>
            <button
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
              <p className="text-slate-600 text-sm font-medium">Total Identitas</p>
              <p className="text-2xl font-bold text-slate-900 mt-1">{totalIdentitas}</p>
            </div>
            <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-200">
              <p className="text-emerald-700 text-sm font-medium">Aktif</p>
              <p className="text-2xl font-bold text-emerald-600 mt-1">{aktif}</p>
            </div>
            <div className="bg-rose-50 rounded-xl p-4 border border-rose-200">
              <p className="text-rose-700 text-sm font-medium">Tidak Aktif</p>
              <p className="text-2xl font-bold text-rose-600 mt-1">{tidakAktif}</p>
            </div>
          </div>
      ...
      <div className="px-8 py-6">
        {filtered.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-slate-500 font-medium">Tidak ada identitas yang ditemukan</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filtered.map((identity) => (
              ...
              >
                ...
                {/* Content Section */}
                <div className="p-4">
                  <h3 className="font-bold text-slate-900 text-sm mb-1">{identity.nama}</h3>
                  <p className="text-slate-600 text-xs mb-3">{identity.jabatan}</p>

                  <div className="space-y-2 mb-4 text-xs">
                    <div className="flex justify-between">
                      <span className="text-slate-500">ID Karyawan:</span>
                      <span className="font-medium text-slate-900">{identity.idKaryawan}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Terdaftar:</span>
                      <span className="font-medium text-slate-900">{identity.terdaftar}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center mb-4">
                    ...
                    >
                      {identity.status === 'Active' ? <Check size={12} /> : <Shield size={12} />}
                      {identity.status}
                    </span>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button className="flex-1 bg-blue-50 hover:bg-blue-100 text-blue-700 py-2 rounded-lg text-xs font-medium transition flex items-center justify-center gap-1">
                      <Cpu size={14} />
                      Encode
                    </button>
                  </div>
                ...
                {deleteConfirm === identity.id && (
                  <div className="absolute inset-0 bg-white rounded-2xl border-2 border-red-300 bg-red-50 flex flex-col items-center justify-center gap-3 p-4">
                    <p className="text-slate-900 font-semibold text-center text-sm">
                      Yakin hapus <span className="font-bold">{identity.nama}</span>?
                    </p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full border border-slate-200">
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-200">
              <h2 className="text-xl font-bold text-slate-900">Tambah Identitas Baru</h2>
              <button
                onClick={() => {
                  setShowAddModal(false);
                  setFormData({ namaLengkap: '', jabatan: '', idKaryawan: '', foto: null });
              ...
              <div>
                ...
                <label className="flex items-center justify-center w-full px-4 py-6 border-2 border-dashed border-slate-300 rounded-lg hover:border-blue-500 hover:bg-blue-50 cursor-pointer transition">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-slate-500 mt-1">PNG, JPG hingga 5MB</p>
                  </div>
                  <input
                    type="file"
                    accept="image/*"
